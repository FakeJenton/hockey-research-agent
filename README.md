...
3. **Leaders + visuals**: a leaderboards page with a scope switch: the 2025-26 season (scoring, xG volume, finishing above expected, save %, team xG share, hottest closers) or the all-time record book (career and single-season leaders across 108 seasons), with an active-players-only filter to see who is climbing the historic lists, and agent answers auto-chart when the result is a time series or ranking (line/bar with a metric picker). All charts are hand-rolled SVG; no chart library.
