...
flowchart LR
    ...
    subgraph dbt["dbt (46 tests)"]
        RAW --> STG[("nhl_stg<br/>13 staging views")]
        STG --> MARTS[("nhl_marts<br/>14 marts")]
    end
...
- **46 dbt tests**: unique/not-null keys, accepted values on position groups and game results, and sanity tests that every one of the 32 teams has exactly 82 game rows.
